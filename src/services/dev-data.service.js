...
async function createDevDataService() {
    ...
    for (let i = 0; i < storeData.users.length; i++) {
        await axios.post("http://localhost:3000/users/registerUser", storeData.users[i])
    }
    //verify them
    let users = [];
    for (let i = 0; i < storeData.users.length; i++) {
        let response = await axios.get(`http://localhost:3000/users/${storeData.users[i].email}`)
    ...
    for (let i = 0; i < users.length; i++) {
        console.log(users[i]);
        await axios.post(`http://localhost:3000/users/verifyRegistration/${users[i].verifyRegistrationToken}`);
    }
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        let result = await axios.post(`http://localhost:3000/users/loginUser`, payload, {
            withCredentials: true 
        });
        console.log(result);
    ...
    for (let i = 0; i < storeData.stores.length; i++) {
        ...
        let response =  await axios.post("http://localhost:3000/stores/store", storeData.stores[i], {
        ...
        if(i === 0) {
            ...
            for (let j = 0; j < storeData.products[i].length; j++) {
                ...
                let result = await axios.post(`http://localhost:3000/stores/product/${storeId}`, storeData.products[i][j], {
                    headers: {'x-access-token': jwtTokenArray[i]}
                });
                console.log(result);
            }
    ...
    for (let i = 0; i < storeIdArray.length; i++) {
        ...
        await axios.post(`http://localhost:3000/stores/addReview/${storeIdArray[i]}`, 
        {
            "rating": "3",
            "text": reviewText
        }, {
